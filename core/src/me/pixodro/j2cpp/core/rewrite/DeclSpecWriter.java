...
public class DeclSpecWriter extends NodeWriter {
  ...
  public DeclSpecWriter(final Scribe scribe, final ASTWriterVisitor visitor, final NodeCommentMap commentMap) {
    super(scribe, visitor, commentMap);
  }

  protected void writeDelcSpec(final IASTDeclSpecifier declSpec) {
  ...
  private String getCPPSimpleDecSpecifier(final ICPPASTSimpleDeclSpecifier simpDeclSpec) {
    return getASTSimpleDecSpecifier(simpDeclSpec.getType(), true);
  }

  private String getCSimpleDecSpecifier(final ICASTSimpleDeclSpecifier simpDeclSpec) {
    return getASTSimpleDecSpecifier(simpDeclSpec.getType(), false);
  }

  private String getASTSimpleDecSpecifier(final int type, final boolean isCpp) {
    switch (type) {
  ...
  private void writeCDeclSpec(final ICASTDeclSpecifier cDeclSpec) {
  ...
  private void writeNamedTypeSpecifier(final ICPPASTNamedTypeSpecifier namedSpc) {
  ...
  private void writeNamedTypeSpecifier(final IASTNamedTypeSpecifier namedSpc) {
    namedSpc.getName().accept(visitor);
  }

  private void writeElaboratedTypeSec(final IASTElaboratedTypeSpecifier elabType) {
    scribe.printStringSpace(getElabTypeString(elabType.getKind()));
    elabType.getName().accept(visitor);
  }

  private String getElabTypeString(final int kind) {
    switch (kind) {
  ...
  private void writeCPPDeclSpec(final ICPPASTDeclSpecifier cppDelcSpec) {
  ...
  private void writeEnumSpec(final IASTEnumerationSpecifier enumSpec) {
    scribe.printStringSpace(Keywords.ENUM);
    enumSpec.getName().accept(visitor);
    scribe.print('{');
    scribe.printSpace();
    final IASTEnumerator[] enums = enumSpec.getEnumerators();
  ...
  private void writeCompositeTypeSpecifier(final IASTCompositeTypeSpecifier compDeclSpec) {
    ...
    final IASTDeclaration[] decls = getMembers(compDeclSpec);

    if (decls.length > 0) {
      for (final IASTDeclaration declaration : decls) {
  ...
  private void writeBaseSpecifiers(final ICPPASTBaseSpecifier specifier) {
  ...
  private String getCPPCompositeTypeString(final int key) {
  ...
  private String getCompositeTypeString(final int key) {
    switch (key) {
  ...
  private void writeDeclSpec(final IASTDeclSpecifier declSpec) {
  ...
  private void writeCPPSimpleDeclSpec(final ICPPASTSimpleDeclSpecifier simpDeclSpec) {
  ...
  private void printQualifiers(final IASTSimpleDeclSpecifier simpDeclSpec) {
  ...
  private void writeCSimpleDeclSpec(final ICASTSimpleDeclSpecifier simpDeclSpec) {
    printQualifiers(simpDeclSpec);
