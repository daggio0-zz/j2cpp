...
public class NameWriter extends NodeWriter {
   ...
   * @param visitor
   */
  public NameWriter(final Scribe scribe, final ASTWriterVisitor visitor, final NodeCommentMap commentMap) {
    super(scribe, visitor, commentMap);
  }

  protected void writeName(final IASTName name) {
  ...
  private boolean isDependentName(final ICPPASTQualifiedName qname, final ICPPASTTemplateId tempId) {
  ...
  private boolean isDependentName(final ICPPASTQualifiedName qname, final ICPPASTTemplateId tempId, final int i) {
    if (i <= 0) {
      return false;
    }
  ...
  private void writeQualifiedName(final ICPPASTQualifiedName qname) {
    if (qname.isFullyQualified()) {
      scribe.print(COLON_COLON);
    }
    final IASTName[] nodes = qname.getNames();
    for (int i = 0; i < nodes.length; ++i) {
      nodes[i].accept(visitor);
      if ((i + 1) < nodes.length) {
        scribe.print(COLON_COLON);
